...
void writeResultsToFile(const std::vector<int>& results, const std::string& outputFilename, const std::string& format) {
    ...
    if (format == "csv") {
        ...
        for (int length : results) {
            outputFile << "Read Length: " << length << std::endl;
        }
    } else {
...
int main(int argc, char* argv[]) {
    ...
    for (const std::string& sequence : sequences) {
        ...
        readLengths.push_back(length);
    }

    if (outputDestination == "terminal") {
        // Print results to the terminal
        printResultsToTerminal(readLengths);
        ...
        writeResultsToFile(readLengths, outputFilename, outputFormat);
    } else {
